        ...
        try {
            ...
            if ("addSaldo".equals(action)) {
                ...
                if (customer != null) {
                    ...
                    if (isSuccess) {
                        response.sendRedirect(request.getContextPath() + "/customerr/profile?success=true");
                    } else {
                        response.sendRedirect(request.getContextPath() + "/customerr/addSaldo?error=true");
                    }
                } else {
                    // Username tidak ditemukan, kirim parameter error
                    response.sendRedirect(request.getContextPath() + "/customerr/addSaldo?error=true");
                }
                ...
                if (isSuccess) {
                    response.sendRedirect(request.getContextPath() + "/customerr/profile?successUpdateProfile=true");
                } else {
                    response.sendRedirect(request.getContextPath() + "/customerr/addSaldo?error=true");
                }
