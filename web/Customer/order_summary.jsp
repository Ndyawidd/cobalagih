                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table>
                                <tr>
                                    <td>
                                        <form action="<%= request.getContextPath() %>/order" method="post" class="mt-4">
                                            <input type="hidden" name="action" value="pay">
                                            <input type="hidden" name="orderId" value="${order.orderId}">
                                            <button type="submit" class="btn btn-success" ${order.status == 'confirmed' || 'cancel' ? 'disabled' : ''}>Bayar Sekarang</button>
                                        </form>
                                    </td>
                                    <td>
                                        <form action="<%= request.getContextPath() %>/order" method="post" class="mt-4">
                                            ...
                                            <input type="hidden" name="orderId" value="${order.orderId}">
                                            <button type="submit" class="btn btn-danger" ${order.status == 'confirmed' || 'cancel' ? 'disabled' : ''}>Cancel</button>
                                        </form>
                                    </td>
                                </tr>
                            </table>    
